...
A free, open-source AI chat app with multi-model support. This guide covers how to install and run the app on different platforms, including Docker deployment options.
...
The app supports BYOK functionality, allowing users to securely store and use their own API keys for AI providers. To enable this feature, you need to configure an encryption key for secure storage of user API keys.
...
Ollama allows you to run AI models locally on your machine. The app has built-in support for Ollama with automatic model detection.
...
The app automatically detects all models available in your Ollama installation. No additional configuration is needed!

**Features:**

- **Automatic Model Detection**: The app scans your Ollama instance and makes all models available
...
By default, the app connects to Ollama at `http://localhost:11434`. This works for local installations.
...
The app includes a settings interface where you can:
...
For a complete Docker setup with both the app and Ollama:
...
docker run -d -v ollama:/root/.ollama -p 11434:11434 --name ollama ollama/ollama
docker run -p 3000:3000 -e OLLAMA_BASE_URL=http://ollama:11434 app
```
...
1. Refresh the models list in settings
2. Check Ollama has models: `ollama list`
3. Restart the app if models were added after startup
...
The app requires Supabase for authentication and storage. Follow these steps to set up your Supabase project:
...
For a complete setup with both the app and Ollama running locally, use the provided `docker-compose.ollama.yml`:

```bash
# Start both the app and Ollama services
...
- **Proper networking** between the app and Ollama
- **Volume persistence** for Ollama models

The Ollama service will be available at `http://localhost:11434` and the app will automatically detect all available models.
...
You can customize various aspects of the app by modifying the configuration files:
